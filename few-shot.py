...
#texts_nw=pkl.load(open("/home/user/Desktop/payel/prompt_2/python_code/texts.pkl","rb"))
...
def make_result_file(dataset_path,output_path_result,output_path_probability,classification_report_file_path,variable,texts,out_path):
    ...
    #my_template=[ManualTemplate(tokenizer=tokenizer, text='{"placeholder":"text_a"} verify if the claim {"placeholder":"text_b"} is supportive or refutive? {"mask"}.'),ManualTemplate(tokenizer=tokenizer, text='{"placeholder":"text_a"} evaluate the validity of the claim that {"placeholder":"text_b"} by determining if it is {"mask"}?') ,ManualTemplate(tokenizer=tokenizer, text='{"placeholder":"text_a"} the truthfulness of the claim {"placeholder":"text_b"} may be confirmed or disproven. This statement is a {"mask"}.'),ManualTemplate(tokenizer=tokenizer, text='{"placeholder":"text_a"} the validity of {"placeholder":"text_b"} can be confirmed as {"mask"}.'),ManualTemplate(tokenizer=tokenizer, text='{"placeholder":"text_a"} then it is {"mask"} that {"placeholder":"text_b"}.')]
...
if __name__ == "__main__":
  ...
  parser.add_argument("--variable",
                      type=int,
                      default = 1,
                      help="Give 1,2,3,4 according to shots",
                      )
